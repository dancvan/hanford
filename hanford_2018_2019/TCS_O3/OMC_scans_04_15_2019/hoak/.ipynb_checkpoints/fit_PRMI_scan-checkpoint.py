...
def fit_OMC_scan(pzt_raw, dcpd, pzt_fs, dcpd_fs, FSR, HOMs, f1, f2):
    ...
    if len(seg_peaks)>=4:
        ...
        for j in range(len(seg_peaks)):
            ...
            for k in range(j+1,len(seg_peaks)):
                ...
                print pzt[idx2], pzt[idx1], dcpd[idx1]/dcpd[idx2]
                if (pzt[idx2]-pzt[idx1])>10 and (pzt[idx2]-pzt[idx1])<15 and (dcpd[idx1]/dcpd[idx2])<1.3 and (dcpd[idx1]/dcpd[idx2])>0.7:
    ...
    for i in range(4):
        ...
        volts = pzt[sb45_idx[i]-r:sb45_idx[i]+r]
        data = dcpd[sb45_idx[i]-r:sb45_idx[i]+r]
    ...
    fit_volts = sb45_fits[:,1]
    fit_freqs = array([FSR+f2, FSR-f2, f2, -1*f2])

    #fit_volts = array([sb45_fits[0,1], tem00_fits[1,1], sb45_fits[1,1], sb45_fits[2,1], tem00_fits[0,1], sb45_fits[3,1]])
    #fit_freqs = array([FSR+f2, FSR, FSR-f2, f2, 0.0, -1*f2])
    ...
    #lsb9_locs = array([2*HOMs-f1, 3*HOMs-f1, 4*HOMs-f1, 4*HOMs-FSR-f1, 5*HOMs-f1, 5*HOMs-FSR-f1, 6*HOMs-FSR-f1, 7*HOMs-FSR-f1, 8*HOMs-FSR-f1])

    #lsb9_text = ['lsb3', 'lsb4', 'lsb4', 'lsb5', 'lsb5', 'lsb6', 'lsb7', 'lsb8']
    #lsb9_locs = array([3*HOMs-f1, 4*HOMs-f1, 4*HOMs-FSR-f1, 5*HOMs-f1, 5*HOMs-FSR-f1, 6*HOMs-FSR-f1, 7*HOMs-FSR-f1, 8*HOMs-FSR-f1])

    #lsb9_text = ['lsb3', 'lsb4', 'lsb4', 'lsb5', 'lsb5', 'lsb6', 'lsb7']
    #lsb9_locs = array([3*HOMs-f1, 4*HOMs-f1, 4*HOMs-FSR-f1, 5*HOMs-f1, 5*HOMs-FSR-f1, 6*HOMs-FSR-f1, 7*HOMs-FSR-f1])

    #lsb9_text = ['lsb3', 'lsb5', 'lsb5', 'lsb6', 'lsb7']
    #lsb9_locs = array([3*HOMs-f1, 5*HOMs-f1, 5*HOMs-FSR-f1, 6*HOMs-FSR-f1, 7*HOMs-FSR-f1])

    #lsb9_text = ['lsb0', 'lsb3', 'lsb5', 'lsb5', 'lsb6', 'lsb7', 'lsb0']
    #lsb9_locs = array([-f1, 3*HOMs-f1, 5*HOMs-f1, 5*HOMs-FSR-f1, 6*HOMs-FSR-f1, 7*HOMs-FSR-f1, FSR-f1])

    lsb9_text = ['lsb0', 'lsb2', 'lsb3', 'lsb5', 'lsb5', 'lsb6', 'lsb7', 'lsb0', 'lsb4', 'lsb4']
    lsb9_locs = array([-f1, 2*HOMs-f1, 3*HOMs-f1, 5*HOMs-f1, 5*HOMs-FSR-f1, 6*HOMs-FSR-f1, 7*HOMs-FSR-f1, FSR-f1, 4*HOMs-FSR-f1, 4*HOMs-f1])
    ...
    # Update the fitting with the lsb9 HOMs
    # previous fitting arrays were [cr0, sb45, hom]

    #start_fit_freqs = fit_freqs
    #fit_volts = hstack((fit_volts, ehom_fits[:,1], lsb9_fits[:,1], usb9_fits[:,1]))
    fit_volts = hstack((fit_volts, lsb9_fits[:,1], usb9_fits[:,1]))
    #fit_volts = hstack((fit_volts, lsb_fits[:,1], usb_fits[:,1], hom_fits[:,1], ehom_fits[:,1], lsb9_fits[:,1], usb9_fits[:,1]))

    #HOM_freqs = remainder(arange(1,Hnum+1)*HOMs,FSR)
    fit_freqs = hstack((fit_freqs, lsb9_locs, usb9_locs))
    #fit_freqs = hstack((fit_freqs, lsb_locs, usb_locs, HOM_freqs, ehom_locs, lsb9_locs, usb9_locs))
    ...
    ehom_fits = zeros((len(ehom_locs),3))
    ehom_errors = zeros((len(ehom_locs),3))

    for j in range(len(ehom_locs)):
        freq = ehom_locs[j]
        ehom_guess = fit_vals[0] + fit_vals[1]*freq + fit_vals[2]*freq**2 + fit_vals[3]*freq**3

        gidx = argmin(abs(pzt - ehom_guess))

        peak_idx = signal.find_peaks_cwt(dcpd[gidx-r:gidx+r],array([r/5])) + gidx-r
        peak_heights = []
        for peak in peak_idx:
            peak_heights.append(dcpd[peak] - sum_model[peak])
        ...
        volts = pzt[pidx-s:pidx+s]
        data = (dcpd - sum_model)[pidx-s:pidx+s]
        ehom_fits[j,:], ehom_errors[j,:] = fit_peak(volts,data,data[s],pzt[pidx])
    ...
    # results of the fits
    mode_fits = vstack((sb45_fits, lsb_fits, usb_fits, lsb9_fits, usb9_fits))
    mode_errors = vstack((sb45_errors, lsb_errors, usb_errors, lsb9_errors, usb9_errors))

    # predicted optical frequencies for the mode fits
    mode_freqs = hstack((start_fit_freqs, lsb_locs, usb_locs, lsb9_locs, usb9_locs))

    mode_text = sb45_text + lsb_text + usb_text + lsb9_text + usb9_text
